...
export default class DotGraph {
  ...
  insertEdge(startNodeName, endNodeName, attributes) {
    var attributesString = toAttributesString(attributes);
    var newEdgeString =
      '    ' + startNodeName + ' -> ' + endNodeName + attributesString;
  ...
  setNodeAttributes(nodeName, attributes) {
    if (this.nodes[nodeName]) {
      this.updateNode(nodeName, attributes)
    }
  }

  updateNodeInStatementList(statementList, id, attributes, newid) {
    statementList.forEach((statement, i) => {
      ...
      if (statement.type === 'attr_stmt') {
        ...
        if (id === statement.node_id.id) {
          ...
          var newNodeString = nodeid + attributesString;
          this.insert(newNodeString);
        } else {
        ...
        let newEdgeString = ""
        let attrmap = {}
        for (i = 0; i < statement.attr_list.length; i++) {
          var attr = statement.attr_list[i]
          attrmap[attr.id] = attr.eq;
        }
        var attributesString = toAttributesString(attrmap);
        let changed = false
        for (i = 0; i < statement.edge_list.length; i++) {
          var edge = statement.edge_list[i]
          var nodeid = edge.id
          if (newid && newid.length > 0 && edge.id === id) {
            nodeid = newid;
            changed = true;
          }
        ...
        if (changed) {
          this.skipLocation(statement, true, true);
          this.insert(newEdgeString)
          if (attributesString.length > 0) {
            this.insert(attributesString)
          }
        } else {
          this.skipLocation(statement, false, true);
        }
        ...
        this.skip('{');
        this.updateNodeInStatementList(
          statement.children,
          id,
          attributes,
          newid,
  ...
  updateNode(node, attributes, newNode) {
    ...
    this.skip('{');
    this.updateNodeInStatementList(this.ast.children, node, attributes, newNode);
    this.skip('}');
  }
        ...
        if (parent.type === 'edge_stmt') {
          if (i > 0) {
            ...
            const nodeNames = nodeIds.map(
              nodeId => nodeId.id + (nodeId.port ? ':' + nodeId.port.id : '')
            );
    ...
    children.forEach((child, i) => {
      ...
      if (child.type === 'node_stmt') {
        ...
        if (typeof child.eq === 'object') {
          ...
          this.str +=
            quoteIdIfNecessary(child.id) + '=' + quoteIdIfNecessary(child.eq);
        }
      } else if (child.type === 'edge_stmt') {
      ...
      if (statement.type === 'attr_stmt') {
        ...
        edgeList.forEach((edgeConnection, i) => {
          if (edgeConnection.type === 'subgraph') {
            ...
            this.deleteComponentInStatementList(
              [subgraph],
              type,
              id,
              edgeRHSId
            ...
            if (!isFirstStatement) {
              ...
              const splitEdge =
                type === 'edge' &&
                nodeIdLeft === id &&
                nodeIdRight === edgeRHSId;
              const eraseEdge =
                eraseNode || erasedAllEdgeConnections || splitEdge;
              this.skipLocationBetween(
                edgeList[i - 1],
                nodeId,
                eraseEdge,
        ...
        if (erasedLastEdgeStatement) {
          ...
          if (statement.attr_list.length > 0) {
            if (
              !statementSeparators.includes(this.dotSrc[this.index - 1]) &&
  ...
  skipBetween(startIndex, endIndex, erase, ignoreStart) {
    if (ignoreStart) {
      ...
      if (startIndex !== this.index) {
        throw Error(
          'Unexpected index ' + this.index + ', expected ' + startIndex
        );
      }
    }
    if (erase) {
  ...
  eraseBetween(startIndex, endIndex) {
    if (startIndex !== endIndex) {
      this.dotSrc =
        this.dotSrc.slice(0, startIndex) + this.dotSrc.slice(endIndex);
  ...
  insert(string) {
    this.dotSrc =
      this.dotSrc.slice(0, this.index) + string + this.dotSrc.slice(this.index);
